...
const ExportProcedure = () => {
  ...
  const [approveProcedure, setApproveProcedure] = useState();
  ...
  const [totalRecordTb5, setTotalRecordTb5] = useState(0);
  ...
  //xuat excel cac hop dong
 
  console.log('t1',dataTb1.filter((item) => {
    return (
  ...
  const handleExportToExcelContract = async (data) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Danh_sach_xuat_kho");
    XLSX.writeFile(workbook, "Danh_sach_xuat_kho.xlsx");
  };
  ...
  const handleExportToExcelCheckGoods = async (data) => {
    ...
    const tmp = data?.map((e, index) => {
      return {
        ...
        "Số hợp đồng bán	": e?.contract_number_attached,
        "Lô hàng	": e?.batch,
  ...
  const handleExportToExcelImplementer = async (data) => {
...
{   (v === "CONFIRMED" || v === "PENDING") && 
  ...
  const columns4 = [
    ...
    ,
    {
      ...
      dataIndex: "list_device",
      key: "list_device",
      render: (v, r) => <Button
        onClick={(e) => {
          e.stopPropagation()
          setDetailDevice(r?.list_device)
        }}
        type="info"
      >
        Chi tiết
        ...
        <>
          <Space onClick={(e) => e.stopPropagation()}>
            ...
            <Button
              ...
              type="primary"
              onClick={
                () => handleExportToExcelImplementer([r])
              }
      ...
      <Row gutter={[8, 8]}>
        <Col>
          <Button
            ...
            type="primary"
            onClick={() =>
              handleExportToExcelImplementer(implementerProcedures)
            }
          >
            <TbTableExport /> <span className="btn-text">Xuất Excel</span>
          </Button>
      ...
      {openDetail && (
        ...
        {checkGoods && (
          <AddCheckGoods
            ...
            setTabKey={setTabKey}
            setImplementerProcedures={setImplementerProcedures}
          />
        )}
        {reportCheckRequest && (
          <AddReportCheck
...
export default ExportProcedure;
