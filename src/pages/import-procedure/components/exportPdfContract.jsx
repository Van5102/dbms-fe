   ...
   return (
    <Page size="A4" style={styles.page}>
      <View style={styles.pageTitle}>
        <Text>Phiếu đề nghị thực hiện hợp đồng</Text>
      </View>
...
<Text style={styles.commonText}>
    ...
    <View style={styles.pageSmallTitle}>
      ...
      <View style={[styles.tableRow]}>
        ...
        <Text style={styles.tableCell}>Thiết bị</Text>
      ...
      {data?.devices.map((row, index) => (
        <View key={index} style={styles.tableRow}>
          <Text style={[styles.tableCell, row.attachment_id == null && styles.boldText]}>{index+1}</Text>
          <Text style={[styles.tableCell, row.attachment_id == null && styles.boldText]}>{row.device_name}</Text>
          <Text style={[styles.tableCell, row.attachment_id == null && styles.boldText]}>{row.quantity}</Text>
...
<View style={styles.pageSectionTitle}>
      ...
      <div>
      <View>
      <Text style={styles.commonText}>Số hợp đồng: {data?.contract_number}</Text>
      <Text style={styles.commonText}>Thông tin liên hệ: {data?.contact}</Text>
      <Text style={styles.commonText}>Mô tả: {data?.description}</Text>
   
      <Text style={styles.commonText}>Chứng từ cung cấp: </Text>
<Text style={styles.commonText}>Hoá đơn GTGT: {data?.invoice_VAT != null ? "Có" : "Không"}</Text>
      ...
      <div>
      <View>
      <Text style={styles.commonText}>Ngày hợp đồng: {moment(parseInt(data?.contract_day) * 1000).format(
         DATETIME_FORMAT)}</Text>
       <Text style={styles.commonText}>SDT liên hệ HD:{data?.contract_phone_number}</Text>
       <Text style={styles.commonText}>Email liên hệ: {data?.contract_email}</Text>
